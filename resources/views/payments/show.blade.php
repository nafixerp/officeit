@extends('layouts.app')
@section('title', 'Payment #' . $payment->payment_number)

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3">Payment #{{ $payment->payment_number }}</h1>
    <div class="btn-group">
        <a href="{{ route('payments.index') }}" class="btn btn-outline-secondary">Back to List</a>
        <a href="{{ route('payments.edit', $payment) }}" class="btn btn-outline-primary">Edit</a>
        <a href="{{ route('payments.print', $payment) }}" class="btn btn-outline-dark" target="_blank"><i class="fas fa-print"></i> Print</a>
    </div>
</div>

<div class="card mb-4">
    <div class="card-header d-flex justify-content-between">
        <h5 class="mb-0">Payment Details</h5>
        @php
            $badgeClass = match($payment->status) {
                'Draft' => 'bg-secondary', 'Confirmed' => 'bg-success',
                'Cancelled' => 'bg-danger', default => 'bg-secondary',
            };
        @endphp
        <span class="badge {{ $badgeClass }} fs-6">{{ $payment->status }}</span>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-3">
                <p class="mb-1 text-muted">Payment Number</p>
                <p class="fw-bold">{{ $payment->payment_number }}</p>
            </div>
            <div class="col-md-3">
                <p class="mb-1 text-muted">Date</p>
                <p class="fw-bold">{{ \Carbon\Carbon::parse($payment->date)->format('d M Y') }}</p>
            </div>
            <div class="col-md-3">
                <p class="mb-1 text-muted">Supplier</p>
                <p class="fw-bold">{{ $payment->supplier->name ?? '-' }}</p>
            </div>
            <div class="col-md-3">
                <p class="mb-1 text-muted">Payment Mode</p>
                <p class="fw-bold">{{ $payment->payment_mode }}</p>
            </div>
        </div>
        <div class="row mt-3">
            <div class="col-md-3">
                <p class="mb-1 text-muted">Currency</p>
                <p class="fw-bold">{{ $payment->currency ?? 'USD' }}</p>
            </div>
            <div class="col-md-3">
                <p class="mb-1 text-muted">Exchange Rate</p>
                <p class="fw-bold">{{ $payment->exchange_rate ?? 1 }}</p>
            </div>
            <div class="col-md-3">
                <p class="mb-1 text-muted">Amount</p>
                <p class="fw-bold fs-5 text-danger">{{ $payment->currency ?? 'USD' }} {{ number_format($payment->amount, 2) }}</p>
            </div>
            <div class="col-md-3">
                <p class="mb-1 text-muted">Reference Number</p>
                <p class="fw-bold">{{ $payment->reference_number ?? '-' }}</p>
            </div>
        </div>
        <div class="row mt-3">
            <div class="col-md-3">
                <p class="mb-1 text-muted">Bank Account</p>
                <p class="fw-bold">{{ $payment->bankAccount->account_name ?? '-' }}</p>
            </div>
            <div class="col-md-9">
                <p class="mb-1 text-muted">Narration</p>
                <p class="fw-bold">{{ $payment->narration ?? '-' }}</p>
            </div>
        </div>
    </div>
</div>
@endsection
