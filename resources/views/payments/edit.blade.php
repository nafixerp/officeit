            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <select name="payment_mode" id="payment_mode" class="form-select" required onchange="toggleAccountFields()">
                        ...
                        <option value="Transfer" {{ old('payment_mode', $payment->payment_mode) == 'Transfer' ? 'selected' : '' }}>Transfer</option>
                        <option value="Online" {{ old('payment_mode', $payment->payment_mode) == 'Online' ? 'selected' : '' }}>Online</option>
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-1">
                    ...
                    <input type="number" name="exchange_rate" id="exchange_rate" class="form-control" value="{{ old('exchange_rate', $payment->exchange_rate) }}" step="0.000001" min="0">
                </div>
                <div class="col-md-3">
                    <label for="amount" class="form-label">Amount <span class="text-danger">*</span></label>
                    <input type="number" name="amount" id="amount" class="form-control" value="{{ old('amount', $payment->amount) }}" step="0.01" min="0" required>
                </div>
                <div class="col-md-3">
                    <label for="reference_number" class="form-label">Reference Number</label>
                    <input type="text" name="reference_number" id="reference_number" class="form-control" value="{{ old('reference_number', $payment->reference_number) }}">
                </div>
                <div class="col-md-3" id="bankAccountField">
                    ...
                    <select name="bank_account_id" id="bank_account_id" class="form-select">
                        ...
                        @foreach($bankAccounts ?? [] as $account)
                            <option value="{{ $account->id }}" {{ old('bank_account_id', $payment->bank_account_id) == $account->id ? 'selected' : '' }}>{{ $account->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3" id="cashAccountField" style="display:none;">
                    ...
                    <select name="cash_account_id" id="cash_account_id" class="form-select">
                        ...
                        @foreach($cashAccounts ?? [] as $account)
                            <option value="{{ $account->id }}" {{ old('cash_account_id', $payment->cash_account_id) == $account->id ? 'selected' : '' }}>{{ $account->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-12">
                    <label for="narration" class="form-label">Narration</label>
                    <textarea name="narration" id="narration" class="form-control" rows="2">{{ old('narration', $payment->narration) }}</textarea>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered mb-0">
                ...
                <tbody id="allocationBody">
                    ...
                    @php $allocated = $payment->allocations->where('invoice_id', $inv->id)->first(); @endphp
                    <tr>
                    ...
                    @empty
                    <tr><td colspan="7" class="text-center text-muted">No unpaid invoices found.</td></tr>
                    @endforelse
                </tbody>
                <tfoot>
                    <tr class="table-info"><td colspan="6" class="text-end fw-bold">Total Allocated:</td><td class="fw-bold" id="totalAllocated">0.00</td></tr>
                </tfoot>
            </table>
        </div>
    </div>
...
function loadUnpaidInvoices() {
    ...
    fetch(`/api/suppliers/${supplierId}/unpaid-invoices`)
        .then(r => r.json()).then(data => {
            ...
            data.forEach(inv => { const bal = (inv.total_amount - inv.paid_amount).toFixed(2); tbody.innerHTML += `<tr><td>${inv.invoice_number}</td><td>${inv.supplier_invoice_number||'-'}</td><td>${inv.date}</td><td class="text-end">${parseFloat(inv.total_amount).toFixed(2)}</td><td class="text-end">${parseFloat(inv.paid_amount).toFixed(2)}</td><td class="text-end">${bal}</td><td><input type="hidden" name="allocations[${inv.id}][invoice_id]" value="${inv.id}"><input type="number" name="allocations[${inv.id}][amount]" class="form-control form-control-sm allocation-amount" value="0" min="0" max="${bal}" step="0.01" onchange="updateAllocationTotal()"></td></tr>`; });
        }).catch(() => {});
}
function updateAllocationTotal() { let t=0; document.querySelectorAll('.allocation-amount').forEach(e=>{t+=parseFloat(e.value)||0;}); document.getElementById('totalAllocated').textContent=t.toFixed(2); }
