        ...
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <input type="number" name="amount" id="amount" class="form-control" value="{{ old('amount') }}" step="0.01" min="0" required onchange="calcTotal()">
                </div>
                <div class="col-md-2">
                    <label for="tax_rate" class="form-label">Tax Rate (%)</label>
                    <input type="number" name="tax_rate" id="tax_rate" class="form-control" value="{{ old('tax_rate', 0) }}" step="0.01" min="0" onchange="calcTotal()">
                </div>
                <div class="col-md-2">
                    <label class="form-label">Tax Amount</label>
                    <input type="text" id="taxAmountDisplay" class="form-control" readonly value="0.00">
                ...
                <div class="col-md-2" id="bankAccField">
                    ...
                    <select name="bank_account_id" id="bank_account_id" class="form-select">
                        ...
                        @foreach($bankAccounts ?? [] as $acc)
                            <option value="{{ $acc->id }}" {{ old('bank_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2" id="cashAccField" style="display:none;">
                    ...
                    <select name="cash_account_id" id="cash_account_id" class="form-select">
                        ...
                        @foreach($cashAccounts ?? [] as $acc)
                            <option value="{{ $acc->id }}" {{ old('cash_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
