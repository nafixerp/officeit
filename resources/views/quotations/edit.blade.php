...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3">Edit Quotation #{{ $quotation->quotation_number }}</h1>
        ...
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <input type="date" name="date" id="date" class="form-control" value="{{ old('date', $quotation->date) }}" required>
                </div>
                <div class="col-md-2">
                    <label for="currency" class="form-label">Currency <span class="text-danger">*</span></label>
                    <select name="currency" id="currency" class="form-select" required>
                        @foreach($currencies ?? ['USD','EUR','GBP','AED','SAR','INR'] as $cur)
                            <option value="{{ $cur }}" {{ old('currency', $quotation->currency) == $cur ? 'selected' : '' }}>{{ $cur }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                    <label for="exchange_rate" class="form-label">Exchange Rate</label>
                    <input type="number" name="exchange_rate" id="exchange_rate" class="form-control" value="{{ old('exchange_rate', $quotation->exchange_rate) }}" step="0.000001" min="0">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    ...
                    <input type="date" name="validity_date" id="validity_date" class="form-control" value="{{ old('validity_date', $quotation->validity_date) }}">
                </div>
                <div class="col-md-10">
                    <label for="remarks" class="form-label">Remarks</label>
                    <textarea name="remarks" id="remarks" class="form-control" rows="2">{{ old('remarks', $quotation->remarks) }}</textarea>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-4">
        ...
        <div class="table-responsive">
            <table class="table table-bordered mb-0" id="itemsTable">
                ...
                <tbody id="itemsBody">
                    @foreach($quotation->items as $idx => $qItem)
                    <tr class="item-row" data-index="{{ $idx }}">
                        <td>
                            <select name="items[{{ $idx }}][item_id]" class="form-select form-select-sm">
                                <option value="">Select Item</option>
                                @foreach($items ?? [] as $item)
                                    <option value="{{ $item->id }}" {{ $qItem->item_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td><input type="text" name="items[{{ $idx }}][description]" class="form-control form-control-sm" value="{{ $qItem->description }}"></td>
                        <td><input type="number" name="items[{{ $idx }}][quantity]" class="form-control form-control-sm qty" value="{{ $qItem->quantity }}" min="0" step="any" onchange="calculateRow(this)"></td>
                        <td><input type="number" name="items[{{ $idx }}][rate]" class="form-control form-control-sm rate" value="{{ $qItem->rate }}" min="0" step="any" onchange="calculateRow(this)"></td>
                        <td><input type="number" name="items[{{ $idx }}][discount]" class="form-control form-control-sm discount" value="{{ $qItem->discount }}" min="0" step="any" onchange="calculateRow(this)"></td>
                        <td>
                            <select name="items[{{ $idx }}][tax_rate_id]" class="form-select form-select-sm tax-rate" onchange="calculateRow(this)">
                                ...
                                @foreach($taxRates ?? [] as $tax)
                                    <option value="{{ $tax->id }}" data-rate="{{ $tax->rate }}" {{ $qItem->tax_rate_id == $tax->id ? 'selected' : '' }}>{{ $tax->name }} ({{ $tax->rate }}%)</option>
                                @endforeach
                            </select>
                        </td>
                        <td><input type="text" class="form-control form-control-sm tax-amount" readonly value="{{ number_format($qItem->tax_amount, 2) }}"></td>
                        <td><input type="text" class="form-control form-control-sm net-amount" readonly value="{{ number_format($qItem->net_amount, 2) }}"></td>
                ...
                <div class="col-md-4">
                    <table class="table table-sm">
                        <tr>
                            ...
                            <td class="text-end" id="subtotal">{{ number_format($quotation->subtotal, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="fw-bold">Total Tax:</td>
                            <td class="text-end" id="totalTax">{{ number_format($quotation->total_tax, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="fw-bold">Total Discount:</td>
                            <td class="text-end" id="totalDiscount">{{ number_format($quotation->total_discount, 2) }}</td>
                        </tr>
                        <tr class="table-primary">
                            <td class="fw-bold">Grand Total:</td>
                            <td class="text-end fw-bold" id="grandTotal">{{ number_format($quotation->total_amount, 2) }}</td>
                        </tr>
                    </table>
