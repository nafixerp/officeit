...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h3">Create Exchange Rate</h1>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('exchange-rates.store') }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('from_currency_id') is-invalid @enderror" id="from_currency_id" name="from_currency_id" required>
                        ...
                        @foreach($currencies as $currency)
                            <option value="{{ $currency->id }}" {{ old('from_currency_id') == $currency->id ? 'selected' : '' }}>{{ $currency->code }} - {{ $currency->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('to_currency_id') is-invalid @enderror" id="to_currency_id" name="to_currency_id" required>
                        <option value="">Select Currency</option>
                        @foreach($currencies as $currency)
                            <option value="{{ $currency->id }}" {{ old('to_currency_id') == $currency->id ? 'selected' : '' }}>{{ $currency->code }} - {{ $currency->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" step="0.000001" class="form-control @error('rate') is-invalid @enderror" id="rate" name="rate" value="{{ old('rate') }}" required>
            ...
            <button type="submit" class="btn btn-primary">Create Exchange Rate</button>
